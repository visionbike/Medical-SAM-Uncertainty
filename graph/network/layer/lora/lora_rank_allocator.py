...
class RankAllocator:
    ...
    Args:
        ...
        beta2 (`float`): The hyperparameter of EMA for uncertainty quantification.
        total_step (`int`): The total training steps, correctly configured before training.
        target_total_rank (`Optional[int]`): The specified final total rank.
        tb_writer (`SummaryWriter`): Tensorboard SummaryWriter.
        tb_writer_loginterval (`int`): The logging interval of SummaryWriter.
    """
    def __init__(
        self, model,
        lora_r:int,
        ...
        self.log_interval = tb_writer_loginterval

        assert (1 > self.beta1 > 0)
        assert (1 > self.beta2 > 0)
    ...
    def schedule_threshold(self, step:int):
        ...
        if step <= initial_warmup:
            ...
            # always masking the same unimportant singular values
            mask_ind = True
        else:
            # Budget decreasing
        ...
        with torch.no_grad():
            ...
            for n,p in model.named_parameters():
                if "lora_E" in n:
                    ...
                    if self.tb_writer is not None and self.global_step%self.log_interval==0:
                        self.tb_writer.add_scalar("Ranknum/%s" % (n,), ranknum, self.global_step)
            ...
            if self.tb_writer is not None and self.global_step%self.log_interval==0:
                self.tb_writer.add_scalar("Budget/total_rank", curr_sum_rank, self.global_step)
                self.tb_writer.add_scalar("Budget/mask_threshold", mask_threshold, self.global_step)
                self.tb_writer.add_scalar("Budget/sum_param", sum_param, self.global_step)
    ...
    def update_and_mask(self, model, global_step):
        ...
        if mask_ind:
            ...
            mask_threshold = None
        self._maybe_tb_writer_log(model)
        return curr_rank, mask_threshold

    def _maybe_tb_writer_log(self, model):
        if self.tb_writer is not None and self.global_step % self.log_interval == 0:
            with torch.no_grad():
                regu_loss = []
                for n, p in model.named_parameters():
...
def compute_orth_regu(model, regu_weight=0.1):
    # The function to compute orthogonal regularization for SVDLinear in `model`.
